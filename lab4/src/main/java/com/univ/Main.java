...
public class Main {
public static int lastIndexOf(String thisstr, String str){
    char lastSymbol = str.charAt(str.length()-1);
    int strLen = str.length();
    for(int i = thisstr.length()-1;i>=0;--i){
        if(thisstr.charAt(i)==lastSymbol){
            int j;
            for(j = str.length()-2;j>0;--j){
                if(thisstr.charAt(--i)!=str.charAt(j)){break;}
            }
            if (j==-1){
                return i+1;
            }
...
//                    }
//
//                    testLang = new MyAnalyzer(fileName, llk);
...
//                    result = testLang.leftRecursNonnerminal();
//                    break;
...
//                    result = testLang.rightRecursNonnerminal();
//                    break;
...
//    }  // кінець main
public static void main(String[] args){
    String tested = "thisisasteststring";
    String str = "test";
    System.out.println(lastIndexOf(tested,str));
}
